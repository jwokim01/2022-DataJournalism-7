...
st.markdown(
"""
투표는 주권자인 국민의 가장 중요한 권리 중 하나로, 우리를 대신하여 일할 국회의원을 선출하는 국회의원 선거는 늘 관심의 대상이 되어 왔다. 그렇지만 이들이 실제로 선출된 후에 무엇에 대해, 얼마나 활동하고 있는지 제대로 알고 있는 학생들은 많지 않을 것이다. 21대 국회 전반기 동안 국회의원에 의해 발의된 법률안은 15,322건에 달하는데, 하루에 약 21건 꼴로 발의된 법안을 우리가 매번 찾아보기는 어려운 일이다.

이번 프로젝트는 크게 두 부분으로 구성된다. 21대 국회 총 16,781건의 국회의원 발의안을 분석하여 첫째, ‘어떤 내용’과 '어떤 특성'의 법률안이 발의되었는지를 살펴본다. 이에 더해, 84,257건 의 뉴스 데이터를 분석한 결과를 토대로 이러한 법률안을 21대 회기 동안의 사회 트렌드와 비교해본다. 둘째, ‘누가’ 이러한 법률안을 발의하였는지를 살펴본다. 이 부분에서는 발의자인 국회의원의 소속 정당과 연령을 기준으로 어떤 법률안이 발의되는지를 살펴본다.
"""
)
...
option3 = st.selectbox('워드클라우드를 확인할 위원회를 선택하세요.', ('외교통일위원회', '국회운영위원회', '법제사법위원회', '정무위원회', '기획재정위원회' , '교육위원회' , '과학기술정보방송통신위원회' , '국방위원회' , '행정안전위원회' , '문화체육관광위원회', '농림축산식품해양수산위원회' , '산업통상자원중소벤처기업위원회' , '보건복지위원회' ,'환경노동위원회' , '국토교통위원회' , '정보위원회' , '여성가족위원회'))
...
이상의 데이터와 그에 따른 설명을 통해 국회의원들이 각 위원회별로 다양한 법률에 관심을 가지고 또 다양한 단어들을 법률안에 사용한다는 사실을 알 수 있다. 국회의원들은 우리 사회 내 다양한 이슈에 관심을 가지고 그에 따른 법률안을 발의하고 있다. 나아가 법률안 전체 내용을 분석한 단어 네트워크 연결망을 살펴보았을 때, 국회의원들이 발의한 법률안에는 이미 존재하는 법률들을 개정하고자 하는 성격이 강하게 존재하고 있으며, 또 지방자치단체를 지원하는 문제에 국회의원들이 큰 관심을 가지고 있다는 사실을 확인해볼 수 있다.
"""
)
...
각 위원회의 아젠다에 대해서 언론에서는 어떤 이야기를 했을까? 이 분석에서는 앞서 도출된 위원회별 핵심 키워드를 중심으로 빅카인즈에서 조선일보와 중앙일보, 그리고 한겨레와 경향신문에서 총 84, 257건의 뉴스(기간: 2020.07~2022.06)를 수집하여 가중치순 상위 50개 키워드를 분석하였다.
"""
)

option2 = st.selectbox('언론별 결과를 확인할 위원회를 선택하세요.', ('외교통일위원회', '국회운영위원회', '법제사법위원회', '정무위원회', '기획재정위원회' , '교육위원회' , '과학기술정보방송통신위원회' , '국방위원회' , '행정안전위원회' , '문화체육관광위원회', '농림축산식품해양수산위원회' , '산업통상자원중소벤처기업위원회' , '보건복지위원회' ,'환경노동위원회' , '국토교통위원회' , '정보위원회' , '여성가족위원회'))
...
위와 같이 도출된 보수 언론과 진보 언론 각각의 TOP5 키워드를 중심으로, 각 언론들의 연관 단어 산점도 네트워크를 그렸다. 분석에는 기사 제목을 토대로 워드투벡(word2vec) 모델이 사용됐으며, 가까이 있을수록 연관성이 높다는 의미이다. 각 매체의 자세한 키워드 네트워크는 맨 아래 첨부해놓았다.
""")
...
option2 = st.selectbox('정당/연령별 워드클라우드를 확인할 위원회를 선택하세요.', ('외교통일위원회', '국회운영위원회', '법제사법위원회', '정무위원회', '기획재정위원회' , '교육위원회' , '과학기술정보방송통신위원회' , '국방위원회' , '행정안전위원회' , '문화체육관광위원회', '농림축산식품해양수산위원회' , '산업통상자원중소벤처기업위원회' , '보건복지위원회' ,'환경노동위원회' , '국토교통위원회' , '정보위원회' , '여성가족위원회'))
