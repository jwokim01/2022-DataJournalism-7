...
st.markdown(
"""
투표는 주권자인 국민의 가장 중요한 권리 중 하나로, 우리를 대신하여 일할 국회의원을 선출하는 국회의원 선거는 늘 관심의 대상이 되어 왔다. 그렇지만 이들이 실제로 선출된 후에 무엇에 대해, 얼마나 활동하고 있는지 제대로 알고 있는 학생들은 많지 않을 것이다. 21대 국회 전반기 동안 발의된 법률 발의안은 15,322건에 달하는데, 하루에 약 21건 꼴로 발의된 법안을 우리가 매번 찾아보기는 어려운 일이다.
이번 프로젝트는 총 16,781건의 모든 법률 발의안, 84, 257건의 뉴스 데이터를 모두 분석하여 여러분에게 위원회별로 국회에서 어떤 법률안
을 발의하고 있는지, 그러한 발의안이 우리의 사회 트렌드를 제대로 반영하고 있는지를 확인해보고자 한다 그리고 여기에서 도출된 '소속 정
당'과 '연령'이라는 기준으로 더 구체적으로 법률 발의안을 살펴볼 것이다.
"""
)
...
img = Image.open('보수_버블.png')
st.image(img, caption='보수 매체에서 2년간 가장 많이 제시된 키워드 TOP5', width=350)
img = Image.open('진보_버블.png')
st.image(img, caption='진보 매체에서 2년간 가장 많이 제시된 키워드 TOP5', width=350)
