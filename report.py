...
st.markdown(
"""
투표는 주권자인 국민의 가장 중요한 권리 중 하나로, 우리를 대신하여 일할 국회의원을 선출하는 국회의원 선거는 늘 관심의 대상이 되어 왔다. 그렇지만 이들이 실제로 선출된 후에 무엇에 대해, 얼마나 활동하고 있는지 제대로 알고 있는 학생들은 많지 않을 것이다. 21대 국회 전반기 동안 국회의원에 의해 발의된 법률안은 16,871건에 달하는데, 하루에 약 21건 꼴로 발의된 법안을 우리가 매번 찾아보기는 어려운 일이다.

이번 프로젝트는 크게 두 부분으로 구성된다. 21대 국회 총 16,871건의 국회의원 발의안을 분석하여 첫째, ‘어떤 내용’의 법률안이 발의되었는지를 살펴본다. 이에 더해, 84,257건 의 뉴스 데이터를 분석한 결과를 토대로 이러한 법률 발의안을 21대 회기 동안의 사회 트렌 드와 비교해본다. 둘째, ‘누가’ 이러한 법률안을 발의하였는지를 살펴본다. 이 부분에서는 발의자인 국회의원의 소속 정당과 연령을 기준으로 어떤 법률안이 발의되는지를 살펴본다.
"""
)


st.header("Ⅱ. 법률 발의안 분석과 사회 트랜드 비교")
st.subheader("1) 위원회별 법률 발의안 트랜드")
st.markdown(
"""
...
url = 'https://www.notion.so/image/https%3A%2F%2Fs3-us-west-2.amazonaws.com%2Fsecure.notion-static.com%2F5a009afd-51a2-464e-830d-1a6293e77729%2FUntitled.jpeg?id=db75864f-428a-4cce-81da-60d07e5964cb&table=block&spaceId=4d52c372-d409-4623-8d9e-be8ea7a07538&width=2000&userId=ea99316d-c5a6-453c-a3c9-39814add9672&cache=v2'
st.image(url)
st.markdown("이 그래프는 국회의원들을 대상으로 유의미한 네트워크 연결망을 그린 결과이다. 이때 동명이인은 고려되지 않았으므로 연결망에 포함된 국회의원의 수는 34명 이상이며, 실제로 김병욱 의원과 이수진 의원이 제 21대 국회 내에 각각 2명씩 존재하므로 연결망 속의 국회의원 수는 총 36명이다. 네트워크에서 서로 다른 색은 서로 다른 집단을 나타내는데, 이 연결망에서는 총 네 집단을 찾을 수 있다. 아울러 국회의원 노드의 크기 비교를 통해 각 국회의원이 가지는 비중을, 그리고 국회의원 간의 연결 링크(link)의 두께를 통해 각 국회의원 간의 연결이 얼마나 두터운지도 알 수 있다.")
...
위와 같이 도출된 보수 언론과 진보 언론 각각의 TOP5 키워드를 중심으로, 각 언론들의 연관 단어 산점도 네트워크를 그렸다. 분석에는 워드투벡(word2vec) 모델이 사용됐으며, 가까이 있을수록 연관성이 높다는 의미이다.
""")
...
st.markdown(
"""
본 프로젝트는 제21대 국회의원들이 발의한 법률안을 중심으로, 현재 국회가 실제로 어떤 키워드에 관심을 갖고 일을 하는지 살펴보고자 했다. 먼저, '어떤 내용'의 법률안이 발의됐는지를 살펴보기 위해 위원회별 워드클라우드를 분석했다. 이를 바탕으로 보수/진보 언론별 키워드를 분석해 국회와 사회의 연결성을 살펴보았다. 그 결과... **(국회 위원회별 분석 추가 예정)** 언론은 특정 인물, 지역 등과 관련되거나 시의성이 있는 키워드가 많이 나타났으며, 사안별로 국회 법률안에 비슷하게 나타나는 것(부동산, 노동 등)과 그렇지 않은 것(디지털 교육, 범죄 등)을 확인할 수 있었다. 그 다음으로, '누가' 어떤 법률안을 발의하는지를 정당과 연령을 기준으로 분석했다. 정당의 전체 발의 내역을 워드클라우드로 분석한 결과 더불어민주당과 국민의힘이 거대 양당으로서 국가 전체의 제도나 국민 전반에 영향을 끼치는 기반 시설과 관련한 법안을 많이 발의하였다면, 정의당은 사회 내부의 소수자를 조망하고 이들의 인권이나 안전 등을 보장하고자 하는 법안을 많이 발의하였다. 연령별 효과는 위원회별로 차이가 있었으나 외교통일위원회와 기획재정위원회에서 의미있는 차이를 확인할 수 있었다. 

민주시민으로서 국회와 정치를 이해하는 것은 중요하지만, 국회 안팎에서 쏟아지는 많은 양의 데이터와 이를 포장하는 자극적인 언어들은 국회의 모습을 있는 그대로 바라보기 어렵게 만든다. 이에 본 프로젝트는 빅데이터 분석을 통해 국회를 보다 객관적으로 드러내고자 했다.
(한계와 제언)
