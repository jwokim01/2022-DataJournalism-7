...
이상의 데이터와 그에 따른 설명을 통해 국회의원들이 각 위원회별로 다양한 법률에 관심을 가지고 또 다양한 단어들을 법률안에 사용한다는 사실을 알 수 있었다. 국회의원들은 우리 사회 내 다양한 이슈에 관심을 가지고 그에 따른 법률안을 발의하고 있다. 나아가 법률안 전체 내용을 분석한 단어 네트워크 연결망을 살펴보았을 때, 국회의원들이 발의한 법률안에는 이미 존재하는 법률안들을 개정하고자 하는 성격이 강하게 존재하고 있으며, 또 지방자치단체를 지원하는 문제에 국회의원들이 큰 관심을 가지고 있다는 사실을 확인해볼 수 있었다.
"""
)
...
if st.button("참고) 보수/진보 매체 주요 단어 네트워크"):
    img = Image.open('보수_네트워크.png')
    st.image(img, caption='보수 매체 주요 단어 네트워크')
    img = Image.open('진보_네트워크.png')
